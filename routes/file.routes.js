...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const senderName = user.username || user.companyName;
      const emailTemplate = `<!DOCTYPE html>
    <html>
    <head>
      <meta charset="utf-8">
      <title>${senderName} has just uploaded some files</title>
    </head>
    <body>
      <h1>${senderName} has just uploaded some files</h1>
...
router.get('/clients/:id/:type/documents', requireLoggedIn, loadOwnedClient, async (req, res) => {
...
router.get('/clients/:id/file-alert', requireAccountant, loadOwnedClient, async (req, res) => {
  const user = req.session.currentUser;
  const clientId = req.params.id;
  const client = await Client.findById(clientId)
    .populate('clientFiles')
...
router.post('/clients/:id/:type/documents/:fileId/delete', requireLoggedIn, loadOwnedClient, async (req, res) => {
  const clientId = req.params.id;
  const type = req.params.type;
