...
router.post('/signup', fileUpload.single('image'), async (req, res) => {
  ...
  if (accountType === 'isFree') {
    ...
    res.render('auth/signup', { errorMessage: 'Invalid account type' });
    return;
  }
...
router.post('/user/change-password', requireLoggedIn, async (req, res) => {
  ...
  if (!user) {
    ...
    return res.status(404).render('users/user-edit', {
